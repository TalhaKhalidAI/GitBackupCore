...
git_router = APIRouter(prefix="/git", tags=["Git_Local_Repo_Manager"])
...
async def list_repositories(
    cuser: Dict[str, Any] = Depends(get_current_active_user),
...
async def create_repository(
    ...
    try:
        ...
        logger.info(f"User {cuser.get('id', 'unknown')} created repository '{repo_name}'")
        ...
        return {
            ...
            "data": {
                ...
                "branch": branch_name,
                "path": repo_path,
                "created_by": cuser.get("username", "unknown")
            }
        }
...
async def delete_repository(
    ...
    try:
        ...
        logger.info(f"User {cuser.get('id', 'unknown')} deleted repository '{repo_name}'")
...
async def create_commit_endpoint(
    ...
    try:
        ...
        result = await gcore.create_commit(
            repo_name=repo_name,
            message=message or f"Auto commit by {cuser.get('username', 'unknown')}"
        )
        
        logger.info(f"Commit created in '{repo_name}' by user {cuser.get('id', 'unknown')}: {result['commit']}")
...
async def upload_staging(
    ...
    """Upload a file to staging area (no commit). Use /commits to commit all staged changes."""
    try:
        gcore = get_git_core()
        ...
        if not full_path.startswith(repo_path_abs + os.sep):
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Invalid file path - must be inside repository"
            )
